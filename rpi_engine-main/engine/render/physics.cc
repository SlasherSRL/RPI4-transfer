...
*/
glm::mat4 GetTransform(ColliderId id)
{
    return glm::inverse(colliders.invTransforms[id.index]);
}
RaycastPayload
Raycast(glm::vec3 start, glm::vec3 dir, float maxDistance, uint16_t mask)
{
    ...
    if (ret.hit)
    {
        ...
        glm::mat4 t = glm::inverse(colliders.invTransforms[ret.collider.index]);
        ret.hitNormal = glm::mat3(t) * ret.hitNormal;
    }

    return ret;
}
 ColliderMesh* GetColliderMesh(ColliderMeshId id)
{

    ColliderMesh* mesh = &meshes[id.index];
    return mesh;
}
 void
 SetActive(ColliderId collider, bool acti)
 {
     assert(colliderPool.IsValid(collider));

     colliders.active[collider.index] = acti;
 }
void Physics::ApplyTransformToColliderMesh(ColliderMeshId id, const glm::mat4& transform)
{
    ...
    /*for (auto& tri : mesh.tris)
    {
        for (int i = 0; i < 3; ++i)
        {
            tri.vertices[i]= glm::vec3(transform * glm::vec4(tri.vertices[i], 1.0f));
...
void DebugDrawColliders()
{
    ...
    for (int colliderIndex = 0; colliderIndex < numColliders; colliderIndex++)
    {
        if (colliders.active[colliderIndex])
        {
            ColliderMesh const* const mesh = &meshes[colliders.meshes[colliderIndex].index];
            ...
            auto radius = colliders.positionsAndScales[colliderIndex][3]; //Fraud radius
            //Debug::DrawLine(centerPoint, centerPoint + glm::normalize(glm::vec3(1.0f, 0, 0)) * radius, 3.0f, glm::vec4(1, 0, 0, 1), glm::vec4(1, 0, 0, 1));
        }
